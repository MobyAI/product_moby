...
export type UserData = UserProfile & {
  ...
  accessLevel: "no_access" | "beta" | "paid";
  betaExpiresAt: Timestamp | null;
  betaCode?: string;
  betaActivatedAt?: Timestamp;
...
export async function addUser(profile: UserProfile, isOnboarding: boolean = false) {
  try {
    ...
    // Get current access level from custom claims (set by Cloud Function on signup)
    const tokenResult = await user.getIdTokenResult();
    const accessLevel = tokenResult.claims.accessLevel || "no_access";
    const betaExpiresAt = tokenResult.claims.betaExpiresAt
      ? Timestamp.fromMillis(tokenResult.claims.betaExpiresAt as number)
      : null;
    ...
    const userData: UserData = {
      ...
      // Access control fields - use values from custom claims
      accessLevel: accessLevel as "no_access" | "beta" | "paid",
  ...
  try {
    ...
    auditionsSnapshot.forEach((doc) => {
      ...
      if (options?.dateRange) {
        const auditionDate = auditionData.date?.toDate
          ? auditionData.date.toDate()
          : new Date(auditionData.date);
        if (
          auditionDate < options.dateRange.start ||
          auditionDate > options.dateRange.end
        ) {
      ...
      if (
        ...
        auditionData.auditionType !== options.auditionType
      ) {
        includeInCount = false;
      }
