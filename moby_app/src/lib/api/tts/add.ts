import { fetchElevenTTS } from "@/lib/api/tts";
...
export async function addTTS(
  ...
  try {
    ...
    try {
      ...
      console.log(
        `🔍 No existing TTS for line ${element.index}, generating...`
      );
    }
    ...
    if (
      sanitizeForTTS(latestLineBeforeUpload?.text || "") !==
      sanitizeForTTS(element.text)
    ) {
      console.warn(
        `⚠️ Line ${element.index} changed mid-TTS — discarding blob`
      );
      return element;
    }
    ...
    console.warn(
      `❌ Failed to generate or upload TTS for line ${element.index}`,
      err
    );
    return element;
  }
...
export async function addTTSRegenerate(
  ...
  try {
    ...
    console.warn(
      `❌ Failed to regenerate and upload TTS for line ${element.index}`,
      err
    );
    return element;
  }
