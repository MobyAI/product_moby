...
function getAlignmentClasses(alignment: ToastAlignment = "right") {
  switch (alignment) {
    case "left":
      return {
...
function ToastViewport({
  ...
  const alignment = getAlignmentClasses(toast.alignment);

  return (
    <div
      className={[
        "fixed inset-0 z-[100] pointer-events-none flex items-start",
        alignment.container,
      ].join(" ")}
    ...
    >
      <div
        ...
        className={[
          alignment.toast,
          "mt-2.5 pointer-events-auto relative flex items-center overflow-hidden rounded-xl shadow-sm backdrop-blur-md",
      ...
      >
        ...
        <div className="flex-shrink-0 ml-4">
          <div
            className={`w-7 h-7 rounded-full flex items-center justify-center ${tone.icon}`}
          >
            {icon}
          </div>
        </div>
        ...
        {/* Content */}
        <div className="flex-1 px-4 py-2.5 mr-15">
          <div
            className={`text-sm ${tone.text} font-bold tracking-tight whitespace-nowrap`}
          >
            {toast.header}
          </div>
          {toast.line1 && (
            <div
              className={`mt-0 text-xs ${tone.text} font-medium whitespace-nowrap`}
            >
              {toast.line1}
            </div>
          )}
