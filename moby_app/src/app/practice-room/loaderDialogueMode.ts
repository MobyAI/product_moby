...
import {
  sanitizeForDialogueMode,
  sanitizeForAlignment,
} from "@/lib/helpers/sanitizerTTS";
...
export const getForcedAlignment = async (
  ...
  if (!response.ok) {
    ...
    const errorMessage =
      errorData?.error || `Forced alignment failed: ${response.status}`;
...
const mapAlignmentToLines = (
  ...
  console.log(
    "First 10 alignment words:",
    words.slice(0, 10).map((w) => w.text)
  );
  ...
  for (const line of scenePartnerLines) {
    ...
    console.log(
      `Next 10 alignment words from position ${wordIndex}:`,
      words
    ...
    while (wordIndex < words.length && wordsFound < lineWords.length) {
      ...
      for (const part of alignedWordParts) {
        ...
        if (
          ...
          if (wordsFound === 0) {
            ...
            console.log(
              `  ✓ Found first word "${targetWord}" in "${alignedWordRaw}" at index ${wordIndex}`
            );
          }
          ...
          for (
            let i = 0;
            i < alignedWordParts.length && wordsFound + i < lineWords.length;
            i++
          ) {
            const alignedPart = alignedWordParts[i];
            const currentTarget = lineWords[wordsFound + i].replace(
              /[^\w\s]/g,
              ""
            );

            if (
              alignedPart === currentTarget ||
              (alignedPart.length > 2 &&
                currentTarget.length > 2 &&
                (alignedPart.includes(currentTarget) ||
                  currentTarget.includes(alignedPart)))
            ) {
              localWordsFound++;
              matchedWords.push(`${currentTarget}(from:${alignedWordRaw})`);
              console.log(
                `    Matched "${currentTarget}" from compound word "${alignedWordRaw}"`
              );
            } else {
      ...
      if (!matched && wordsFound > 0) {
        ...
        for (
          let offset = 1;
          offset <= lookAheadWindow && wordIndex + offset < words.length;
          offset++
        ) {
          const futureWordRaw = words[wordIndex + offset].text;
          const futureWord = futureWordRaw
            .toLowerCase()
            .replace(/[^\w\s]/g, "")
            .replace(/'/g, "");

          if (
            futureWord === targetWord ||
            (futureWord.length > 2 &&
              targetWord.length > 2 &&
              (futureWord.includes(targetWord) ||
                targetWord.includes(futureWord)))
          ) {
            console.log(
              `  ⚠️ Found target "${targetWord}" ${offset} words ahead (skipping unexpected: "${alignedWordRaw}" and ${
                offset - 1
              } others)`
            );
      ...
      if (wordsFound === lineWords.length) {
        if (lineStartTime !== undefined && lineEndTime !== undefined) {
          ...
          console.log(
            `  - Time range: ${lineStartTime.toFixed(
              2
            )}s - ${lineEndTime.toFixed(2)}s (${(
    ...
    if (!timingMap.has(line.index)) {
      ...
      if (wordsFound > 0 && lastMatchIndex > 0) {
        ...
        console.log(
          `  - Partial match found, resetting to just after last match at index ${newIndex}`
        );
        ...
        console.log(
          `  - No match found, resetting to last successful line end at index ${newIndex}`
        );
      } else {
  ...
  console.log("\n=== Alignment Summary ===");
  console.log(
    `Successfully matched: ${timingMap.size}/${scenePartnerLines.length} lines`
  );
  console.log(
    `Failed lines:`,
    scenePartnerLines.filter((l) => !timingMap.has(l.index)).map((l) => l.index)
  );
  console.log(`Final word index position: ${wordIndex}/${words.length}`);

  if (timingMap.size > 0) {
    const durations = Array.from(timingMap.values()).map(
      (t) => t.endTime - t.startTime
    );
    console.log(
...
const splitAudioIntoSegments = async (
  ...
  const sortedEntries = Array.from(timingMap.entries()).sort(
    (a, b) => a[0] - b[0]
  );
  ...
  for (let i = 0; i < sortedEntries.length; i++) {
    ...
    console.log(
      `  Original timing: ${timing.startTime.toFixed(
        3
      )}s - ${timing.endTime.toFixed(3)}s`
    );

    const gap = nextEntry ? nextEntry[1].startTime - timing.endTime : 1.0;
    const gapFromPrev = prevEntry
      ? timing.startTime - prevEntry[1].endTime
      : 1.0;

    if (prevEntry) {
      console.log(
        `  Gap from previous line: ${(gapFromPrev * 1000).toFixed(1)}ms`
      );
      if (gapFromPrev < 0) {
        console.log(
          `  ⚠️ OVERLAP DETECTED: Lines ${
            prevEntry[0]
          } and ${lineIndex} overlap by ${Math.abs(gapFromPrev * 1000).toFixed(
            1
          )}ms`
        );
      }
    ...
    if (nextEntry) {
      ...
      if (gapToNext < 0) {
        console.log(
          `  ⚠️ OVERLAP DETECTED: Lines ${lineIndex} and ${
            nextEntry[0]
          } overlap by ${Math.abs(gapToNext * 1000).toFixed(1)}ms`
        );
      }
    }
    ...
    if (gap < 0.1) {
      ...
      if (actualEndPadding === endPaddingSeconds) {
        console.log(
          `  Using full padding: ${(endPaddingSeconds * 1000).toFixed(
            0
          )}ms (gap ${(gap * 1000).toFixed(0)}ms allows it)`
        );
      } else {
        console.log(
          `  Using proportional padding: ${(actualEndPadding * 1000).toFixed(
            0
          )}ms (30% of ${(gap * 1000).toFixed(0)}ms gap)`
        );
      }
    ...
    if (gapFromPrev < 0) {
      ...
      if (actualStartPadding === startPaddingSeconds) {
        console.log(
          `  Start padding: ${(actualStartPadding * 1000).toFixed(
            0
          )}ms (capped at max)`
        );
        ...
        console.log(
          `  Start padding: ${(actualStartPadding * 1000).toFixed(
            0
          )}ms (minimum padding, gap only ${(gapFromPrev * 1000).toFixed(0)}ms)`
        );
      } else {
        console.log(
          `  Start padding: ${(actualStartPadding * 1000).toFixed(
            0
          )}ms (50% of ${(gapFromPrev * 1000).toFixed(0)}ms gap)`
        );
      }
    }
    ...
    if (nextLineStartTime !== undefined) {
      ...
      if (endTime === maxEndTime) {
        console.log(
          `  ⚠️ End time capped by safety margin: ${endTime.toFixed(3)}s (${(
            safetyMargin * 1000
          ).toFixed(0)}ms before next line)`
        );
      }
    }
    ...
    if (nextEntry) {
      const nextStartTime = Math.max(
        0,
        nextEntry[1].startTime - startPaddingSeconds
      );
      ...
      if (endByte >= nextStartByte) {
        ...
        console.log(
          `  ⚠️ Trimmed line ${lineIndex} end to prevent overlap with line ${nextEntry[0]}`
        );
      }
    }
    ...
    if (segmentLength <= 0) {
      console.warn(
        `⚠️ Line ${lineIndex} has invalid segment length: ${segmentLength}`
      );
      continue;
    }
    ...
    const segmentBlob = new Blob([wavHeader, segmentPCM], {
      type: "audio/wav",
    });
...
const splitDialogueIntoBatches = (
  ...
  for (const entry of entries) {
    ...
    if (
      currentCharCount + entryCharCount > maxChars &&
...
export const hydrateScriptWithDialogue = async ({
  ...
  try {
    ...
    // Check if any lines are missing voiceId
    const linesWithoutVoice = linesNeedingHydration.filter(
      (line) => !line.voiceId
    );
    if (linesWithoutVoice.length > 0 && showToast) {
      showToast({
    ...
    const dialogueEntries: DialogueEntry[] = linesNeedingHydration.map(
      (line) => ({
        text: sanitizeForDialogueMode(line.text),
        voiceId: line.voiceId || VOICE_MAPPING.default,
        lineIndex: line.index,
    ...
    batches.forEach((batch, i) => {
      const charCount = batch.reduce((sum, e) => sum + e.text.length, 0);
      console.log(
        `  Batch ${i + 1}: ${batch.length} entries, ${charCount} characters`
      );
      console.log(
        `    Lines ${batch[0].lineIndex} to ${
          batch[batch.length - 1].lineIndex
        } `
      );
    });
    ...
    for (let i = 0; i < batches.length; i++) {
      const batch = batches[i];
      setLoadStage(
        `🎤 Generating dialogue audio ${i + 1}/${batches.length}...`
      );
      console.log(`📤 Generating batch ${i + 1}/${batches.length}...`, batch);
      ...
      const firstBytes = new Uint8Array(
        await batchBlob.slice(0, 20).arrayBuffer()
      );
      console.log(`First bytes:`, firstBytes);
      console.log(
        `✅ Batch ${i + 1} generated (${(batchBlob.size / 1024).toFixed(2)} KB)`
      );
      ...
      console.log(
        `  Audio blob size: ${(batchBlob.size / 1024 / 1024).toFixed(2)}MB (${
          batchBlob.size
        } bytes)`
      );
      console.log(`  Transcript length: ${batchTranscript.length} characters`);
      console.log(
        `  Transcript preview: "${batchTranscript.substring(0, 100)}..."`
      );
      ...
      const batchAlignmentData = await getForcedAlignment(
        audioBlob,
        batchTranscript
      );
      ...
      const batchLines = batch
        .map((entry) =>
          sanitizedLines.find((line) => line.index === entry.lineIndex)
        )
        .filter((line): line is NonNullable<typeof line> => line !== undefined);

      const batchTimingMap = mapAlignmentToLines(
        batchAlignmentData,
        batchLines
      );
      ...
      const batchSegmentMap = await splitAudioIntoSegments(
        audioBlob,
        batchTimingMap
      );
    ...
    console.log(
      `✅ Combined ${segmentMap.size} segments from ${allSegmentMaps.length} batches`
    );
    ...
    // Use the extended type for the updated script
    const updatedScript: (ScriptElement | ScriptElementWithTiming)[] =
      await Promise.all(
        script.map((element) =>
          uploadLimit(async () => {
            if (element.type === "line" && segmentMap.has(element.index)) {
              try {
                const audioBlob = segmentMap.get(element.index)!;
                const timing = timingMap.get(element.index)!;
                ...
                completedOperations++;
                onProgressUpdate?.(
                  calculateWeightedProgress(completedOperations),
                  100
                );
                ...
                console.error(
                  `❌ Failed to upload audio for line ${element.index}:`,
                  err
                );
                Sentry.captureException(err);
                failedLines.push(element.index);
    ...
    const end = performance.now();
    console.log(
      `⏱️ Script hydrated with dialogue mode in ${(end - start).toFixed(2)} ms`
    );
...
const createWeightedProgressCalculator = (
  batchCount: number,
  lineCount: number
) => {
  ...
  return (completedOperations: number) => {
    ...
    if (completedOperations <= totalBatchOperations) {
      ...
      weightedProgress =
        completedBatches * progressPerBatch + batchProgress * progressPerBatch;
    }
