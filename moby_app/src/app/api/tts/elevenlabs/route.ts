...
export async function POST(req: Request) {
    try {
        ...
        if (languageCode && !/^[a-z]{2,3}$/i.test(languageCode)) {
            return NextResponse.json(
                { error: 'Invalid language code. Use ISO 639-1 format (e.g., "en", "fr", "de")' },
                { status: 400 }
            );
        }

        // Validate text normalization option
        if (applyTextNormalization && !['auto', 'on', 'off'].includes(applyTextNormalization)) {
            return NextResponse.json(
        ...
        return new Response(arrayBuffer, {
            ...
            headers: {
                ...
                'Content-Disposition': `inline; filename="tts.${outputFormat?.startsWith('pcm') ? 'pcm' : 'mp3'}"`,
