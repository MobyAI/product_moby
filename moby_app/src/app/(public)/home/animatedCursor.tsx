...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      // Base navy blue #363c54 is approximately H=228°, S=36%, V=33%
      const baseHue = 228 / 360; // ~0.633
      const baseSaturation = 0.36;
      const baseValue = 0.33;

      // Generate slightly lighter shades
      const c = HSVtoRGB(
        baseHue + (Math.random() - 0.5) * 0.02, // hue varies slightly (±0.02)
        baseSaturation * (0.9 + Math.random() * 0.2), // saturation 90-110% of base
        baseValue * (1.1 + Math.random() * 0.3) // value 110-140% of base (lighter)
      );
